...

  {
    ...
    for (uint32_t k = 0; k < 100; ++k) {
...

      {
        ...
        const auto [l1, m1, h1] = tree1.check_overlap(Vec2<BT>{BT(n0 - width), BT(n1 - height)},
                                                      Vec2<BT>{BT(n0 + width), BT(n1 + height)}, h);
        const auto [l2, m2, h2] = MQT2::Detail::naive_tester<double, BT>(
            map, Vec2<BT>{BT(n0 - width), BT(n1 - height)}, Vec2<BT>{BT(n0 + width), BT(n1 + height)}, size, h);
...

  {
    ...
    for (uint32_t k = 0; k < 100; ++k) {
      ...
      for (uint32_t j = 0; j < 100; ++j) {
        ...
        const auto [l1, m1, h1] =
            tree1.check_overlap(Vec2<BT>{n0 - width, n1 - height}, Vec2<BT>{n0 + width, n1 + height}, h);
        const auto [l2, m2, h2] = MQT2::Detail::naive_tester<double>(map, Vec2<BT>{n0 - width, n1 - height},
                                                                     Vec2<BT>{n0 + width, n1 + height}, 6400, h);
...
void MQT2_tester_int() {
...

  {
    ...
    if constexpr (true) {
      const auto [l1, m1, h1] = tree.check_overlap(Vec2<BT>{10, 10}, Vec2<BT>{25, 25}, 1.);
      const auto [l2, m2, h2] =
          MQT2::Detail::naive_tester<uint32_t, BT>(map, Vec2<BT>{10, 10}, Vec2<BT>{25, 25}, 40, 1);
      std::cout << "Test 6: ";
      std::cout << " Res: " << l1 << ", " << m1 << ", " << h1;
      std::cout << " Ex: " << l2 << ", " << m2 << ", " << h2;
      std::cout << (l1 == l2 && m1 == m2 && h1 == h2 ? " passed" : " failed") << std::endl;
...

  {
    ...
    for (uint32_t k = 0; k < 100; ++k) {
...

      {
        ...
        const auto [l1, m1, h1] =
            tree1.check_overlap(Vec2<BT>{n0 - width, n1 - height}, Vec2<BT>{n0 + width, n1 + height}, h);
        const auto [l2, m2, h2] = MQT2::Detail::naive_tester<uint32_t>(map, Vec2<BT>{n0 - width, n1 - height},
                                                                       Vec2<BT>{n0 + width, n1 + height}, size, h);
...
void bench_tree2() {
  ...
  if constexpr (false) {
    for (uint32_t i = 0; i < 2000 * 7680; ++i) map[dist(rand)] = 2.;
  }
...

  {
    file << "tree 1" << std::endl;
    {
      ...
      for (uint32_t i = 1; i < 50; ++i) {
        ...
        for (uint32_t j = 0; j < runs; ++j) {
          const auto [l1, m1, h1] =
              tree.check_overlap(Vec2<BT>{i * 50, i * 50}, Vec2<BT>{7680 - i * 50, 7680 - i * 50}, hh);
          t += l1;
        }
      }
    ...
    //             const auto start = std::chrono::high_resolution_clock::now();
    //             const auto[l1, m1, h1] = tree.check_overlap(Vec2<BT>{i * 50, i * 50}, Vec2<BT>{7680 - i * 50, 7680 -
    //             i * 50}, hh); const std::chrono::duration<double> ee = std::chrono::high_resolution_clock::now() -
    //             start; tmp += ee.count(); t += l1;
    //         }
    ...
    if constexpr (false) {
      ...
      for (uint32_t i = 1; i < 50; ++i) {
        ...
        for (uint32_t j = 0; j < 12; ++j) {
          const auto start        = std::chrono::high_resolution_clock::now();
          const auto [l2, m2, h2] = MQT2::Detail::naive_tester<SCALAR, BT>(
              map, Vec2<BT>{BT(i * 50), BT(i * 50)}, Vec2<BT>{7680 - i * 50, 7680 - i * 50}, 7680, hh);
    ...
    file << "tree 1" << std::endl;
    {
      ...
      for (uint32_t i = 1; i < 50; ++i) {
        ...
        for (uint32_t j = 0; j < 12; ++j) {
          const auto start = std::chrono::high_resolution_clock::now();
          const auto [l1, m1, h1] =
              tree.check_border_overlap(Vec2<BT>{i * 50, i * 50}, Vec2<BT>{7680 - i * 50, 7680 - i * 50}, hh);
          const std::chrono::duration<double> ee = std::chrono::high_resolution_clock::now() - start;
          tmp += ee.count();
          t += l1;
        }
...
void idx_test() {
  const auto idx_n = [](const uint32_t _level, const uint32_t _idx) -> uint32_t { return 4 * _idx + 1; };
  ...
  for (uint32_t i = 5; i < 21; ++i) { std::cout << idx_b(3, i) << std::endl; }
}
